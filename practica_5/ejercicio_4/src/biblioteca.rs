...
impl Biblioteca {
    ...
    fn escribir_copias(&self){
        match File::create("copias.json") {
            Ok(mut file) => {
                let json = serde_json::to_string(&self.disposicion).unwrap();
    ...
    fn escribir_prestamos(&self) {
        match File::create("./prestamos.json") {
            Ok(mut file) => {
                let t_s = serde_json::to_string_pretty(&self.prestamos).unwrap();
                file.write_all(&t_s.as_bytes()).expect("Error al escribir el archivo prestamos.json");
            },
            Err(error) => {
    ...
    pub fn prestamos_a_vencer(&self, dias: u32) -> Vec<&Prestamo> {
        ...
        for p in self.prestamos.iter() {
            if actual_fecha.es_mayor(p.vencimiento.clone()) {
                prestamos.push(p);
            }
        }
...
fn test_biblioteca_prestamos_a_vencer() {
    ...
    b.realizar_prestamo(c.clone(), l.clone(), Fecha::new(17, 6, 2025));
    b.realizar_prestamo(c.clone(), l.clone(), Fecha::new(20, 7, 2025));
    b.realizar_prestamo(c.clone(), l.clone(), Fecha::new(15, 6, 2025));
    b.realizar_prestamo(c.clone(), l.clone(), Fecha::new(18, 6, 2025));
...
fn test_biblioteca_prestamos_vencidos() {
    ...
    b.realizar_prestamo(c.clone(), l.clone(), Fecha::new(5, 6, 2025));
    b.realizar_prestamo(c.clone(), l.clone(), Fecha::new(2, 6, 2025));
    b.realizar_prestamo(c.clone(), l.clone(), Fecha::new(14, 7, 2025));
